...
const list = [
  ...
  },
  {
    ...
    text: "Оборудование может быть адаптировано под ваши запросы, включая окраску</br>и конфигурацию.",
    img: "/icon/ad_6.svg",
  },
];
...
export default function Advantages() {
  ...
  // Optimize animation by checking visibility and using lower frame rate
  useEffect(() => {
    const slider = sliderRef.current;
    if (!slider) return;

    // Observer to check if component is visible in viewport
    ...
    // Use a slower update interval to reduce performance impact
    let scrollInterval;
    if (autoScroll) {
      scrollInterval = setInterval(() => {
        if (isVisibleRef.current && slider && autoScroll) {
          slider.scrollLeft += 1;
          if (slider.scrollLeft >= slider.scrollWidth / 2) {
            slider.scrollLeft = 0;
          }
        }
      }, 33); // ~30fps instead of full requestAnimationFrame speed
    }
  ...
  // Create doubled list for infinite scroll without duplicating DOM elements
  ...
  return (
    <section className="overflow-x-hidden w-full bg-gray-400 py-[40px] xs:py-[40px] sm:py-[40px] md:py-[40px] lg:py-[40px] xl:py-[40px] 2xl:py-[40px] ">
      ...
      >
        {doubledList.map(({ title, text, subtitle, img }, i) => (
          <article
            key={i}
